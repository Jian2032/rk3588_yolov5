...
const unsigned char CRC8_INIT = 0xff; // 二进制中为8个1，进行初始化
...
// 获取CRC8校验值函数
// unsigned char *pchMessage: 指向输入数据缓冲区的指针
// unsigned int dwLength：缓冲区的长度（要处理的字节数）。
// unsigned char ucCRC8：初始 CRC 值
// 
unsigned char CRC::Get_CRC8_Check_Sum(unsigned char *pchMessage,unsigned  int dwLength,unsigned char ucCRC8) 
{ 
	unsigned char ucIndex; // 用于存储当前字节与CRC值异或操作的结果
	// 循环处理数据缓冲区中的每个字节
	while (dwLength--) 
	{ 
		// 将当前CRC值与数据缓冲区中的当前字节进行异或操作
		ucIndex = ucCRC8^(*pchMessage++); 
		// 使用异或操作的结果作为索引，从预计算的CRC查找表中获取新的CRC值
		// 每个循环这里都进行CRC的更新，后面根据新的CRC再进行计算
...
{ 
	unsigned char ucExpected = 0; // 存储期望的CRC校验值
	if ((pchMessage == 0) || (dwLength <= 2)) return 0; // 输入指针不能为空且长度要大于2
	ucExpected = Get_CRC8_Check_Sum (pchMessage, dwLength-1, CRC8_INIT); // 最后一位已经是CRC不需要计算所以 dwLength-1,
	return ( ucExpected == pchMessage[dwLength-1] ); // 将计算的期望CRC与本身数据最后一个字节的CRC对比
} 
...
** Output: True or False (CRC Verify Result) 
*/ 
// unsigned char *pchMessage：指向要验证的数据缓冲区的指针。
// unsigned int dwLength：数据缓冲区的长度，包括数据和CRC校验值。
